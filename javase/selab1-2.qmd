...
<div id="Goal" class="tabcontentw3">
  ...
  Now, it this version v2.0 you will storage the data on local JSON and use the `factory` pattern. For that, your will need to create the ``Account`` ``interface`` and two subclasses: ``SavingsAccount`` and ``CheckingAccount``.</p>
</div>
...
<div id="Optional" class="tabcontentw3">
  ...
  Add `Credit Card` class and create the **new feature** `buy Credit Cart` in `ÀccountManager`.</p><p class="indent">
